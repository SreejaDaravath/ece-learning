document.addEventListener('DOMContentLoaded', function(){
  ...
  const els = {
    ...
    skipLevelBtn: document.getElementById('skipLevel'),
  ...
  function updateUI(){
    ...
    updateCircuitDiagram(R1, R2, vout);
    
    const diff = Math.abs(vout - targetV);
    const ok = diff <= (tolFrac * Vin);
    if(ok && !solved){
      console.log('🎯 Target matched! Vout:', vout.toFixed(3), 'Target:', targetV.toFixed(2), '±', (tolFrac * Vin).toFixed(2));
      solved = true;
      onSolve();
    }
    if(ok){
  ...
  // Advanced Circuit Simulation with Canvas
  const circuitCanvas = document.getElementById('circuitCanvas');
  const circuitCtx = circuitCanvas ? circuitCanvas.getContext('2d') : null;
  let electrons = [];
  let circuitAnimationId = null;
  
  function updateCircuitDiagram(r1, r2, vout){
    if(!circuitCtx) return;
    
    // Calculate current (I = V / R_total)
    const totalR = r1 + r2;
    const current = totalR > 0 ? (Vin / totalR) * 1000 : 0; // in mA
    
    // Update current display
    const currentDisplay = document.getElementById('currentDisplay');
    if(currentDisplay) currentDisplay.textContent = current.toFixed(2);
    
    // Initialize electrons for animation
    if(electrons.length === 0 || Math.random() < 0.05){
      initElectrons(current);
    }
    
    // Draw circuit
    drawCircuit(r1, r2, vout, current);
  }
  
  function initElectrons(current){
    electrons = [];
    const numElectrons = Math.max(5, Math.min(20, Math.floor(current / 2)));
    for(let i = 0; i < numElectrons; i++){
      electrons.push({
        segment: Math.floor(Math.random() * 4), // 0-3 segments
  ...
  function drawCircuit(r1, r2, vout, current){
    if(!circuitCtx) return;
    ...
    const battery = {x: 80, y: centerY};
    const r1Start = {x: 200, y: centerY};
    const r1End = {x: 320, y: centerY};
    const junction = {x: 420, y: centerY};
    const r2Start = {x: junction.x, y: centerY + 20};
    const r2End = {x: junction.x, y: centerY + 140};
    const ground = {x: junction.x, y: centerY + 180};
    const returnWire = {x: 80, y: ground.y};
    
    // Draw wires with glow effect
    circuitCtx.strokeStyle = '#475569';
    circuitCtx.lineWidth = 6;
    circuitCtx.lineCap = 'round';
    circuitCtx.shadowBlur = 10;
    circuitCtx.shadowColor = '#60a5fa';
    
    // Wire segments (defining electron path)
    const segments = [
      {from: battery, to: r1Start, color: '#60a5fa'}, // Battery to R1
      {from: r1End, to: junction, color: '#60a5fa'},  // R1 to junction
      {from: r2Start, to: r2End, color: '#60a5fa'},   // Junction to R2
      {from: r2End, to: returnWire, color: '#60a5fa'} // R2 to ground, back to battery
    ];
    
    // Draw all wire segments
    segments.forEach(seg => {
      circuitCtx.beginPath();
      circuitCtx.moveTo(seg.from.x, seg.from.y);
      circuitCtx.lineTo(seg.to.x, seg.to.y);
      circuitCtx.strokeStyle = seg.color;
      circuitCtx.stroke();
    });
    ...
    // Draw Battery (Vin)
    drawBattery(circuitCtx, battery.x, battery.y, Vin);
    
    // Draw R1 Resistor
    drawResistor(circuitCtx, r1Start.x, r1Start.y, r1End.x, r1End.y, r1, 'R1');
    
    // Draw Junction point (Vout measurement)
    drawJunction(circuitCtx, junction.x, junction.y, vout);
    
    // Draw R2 Resistor (vertical)
    drawResistor(circuitCtx, r2Start.x, r2Start.y, r2End.x, r2End.y, r2, 'R2');
    
    // Draw Ground symbol
    drawGround(circuitCtx, ground.x, ground.y);
    
    // Draw animated electrons
    drawElectrons(circuitCtx, segments, current);
    
    // Continue animation
    if(current > 0){
      circuitAnimationId = requestAnimationFrame(() => drawCircuit(r1, r2, vout, current));
    }
  }
  ...
  function drawResistor(ctx, x1, y1, x2, y2, resistance, label){
    ...
    const rectX = midX - width/2;
    const rectY = midY - height/2;
    ...
    // Value
    ctx.font = '12px Arial';
    const resistanceText = resistance >= 1000 ? 
      (resistance/1000).toFixed(1) + 'kΩ' : 
      resistance + 'Ω';
    ctx.fillText(resistanceText, midX, midY + 8);
  }
  
  function drawJunction(ctx, x, y, voltage){
    // Junction point (red dot)
    ...
    ctx.fillRect(x + 15, y - 20, 70, 25);
    
    ctx.fillStyle = '#fbbf24';
    ctx.font = 'bold 14px Arial';
    ctx.textAlign = 'left';
    ctx.fillText('Vout', x + 20, y - 8);
    ctx.fillStyle = '#34d399';
    ctx.fillText(voltage.toFixed(2) + 'V', x + 20, y + 5);
  }
  ...
  function drawGround(ctx, x, y){
    ...
    ctx.textAlign = 'center';
    ctx.fillText('GND', x, y + 32);
  }
  
  function drawElectrons(ctx, segments, current){
    if(current <= 0) return;
    
    electrons.forEach(electron => {
      // Update electron position
      electron.progress += electron.speed * (current / 10);
      if(electron.progress >= 1){
        electron.progress = 0;
        electron.segment = (electron.segment + 1) % segments.length;
      }
      
      // Get current segment
      const seg = segments[electron.segment];
      const x = seg.from.x + (seg.to.x - seg.from.x) * electron.progress;
      const y = seg.from.y + (seg.to.y - seg.from.y) * electron.progress;
  ...
  function onSolve(){
    ...
    localStorage.setItem('noteGateScore', score);
    ...
    if(currentLevel < levels.length){
      ...
      if(els.nextLevelBtn){
        ...
        els.nextLevelBtn.style.visibility = 'visible';
        els.nextLevelBtn.style.opacity = '1';
        console.log('✅ Next Level button shown for level ' + nextLevelNum);
        console.log('   Button display:', els.nextLevelBtn.style.display);
        console.log('   Button visibility:', els.nextLevelBtn.style.visibility);
      } else {
  ...
  function resetLevelState(){
    ...
    // Reset sliders
    if(els.r1) els.r1.value = 1000;
    if(els.r2) els.r2.value = 1000;
  ...
  function nextLevel(){
    ...
    localStorage.setItem('noteGateCurrentLevel', currentLevel);
    localStorage.setItem('noteGateScore', score);
    console.log('💾 Progress saved: Level ' + currentLevel + ', Score: ' + score);
  ...
  function skipLevel(){
    console.log('⏭️ Skip Level button clicked. Current level: ' + currentLevel);
    if(currentLevel >= levels.length){
      showToast('🏆 You are already at the final level!');
      return;
    }
    // Unlock the next level without requiring solving current one
    currentLevel += 1;
    const unlocked = JSON.parse(localStorage.getItem('noteGateUnlocked') || '[]');
    if(!unlocked.includes(currentLevel)){
      unlocked.push(currentLevel);
      localStorage.setItem('noteGateUnlocked', JSON.stringify(unlocked));
      console.log('🔓 Level ' + currentLevel + ' unlocked via skip');
    }
    
    console.log('➡️ Skipping to level ' + currentLevel);
    if(els.level) els.level.textContent = currentLevel;
    ...
    showToast('⏭️ Level skipped! No points awarded.');
    startLevel(currentLevel);
  }
  ...
  function startLevel(level){
    ...
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('🎮 Starting Level ' + level + ' (' + topic.toUpperCase() + ')');
  ...
  function showAnalogControls(){
    ...
    // Hide digital game UI
    const dg = document.getElementById('digitalGame');
    if(dg) dg.style.display = 'none';
    
    // Hide switch controls (digital)
    const switchControls = document.getElementById('switchControls');
    if(switchControls) switchControls.style.display = 'none';
    
    // Show analog info (Vin, Target Vout, etc.)
    const infoItems = document.querySelectorAll('.info > div');
    infoItems.forEach(item => {
      item.style.display = 'block'; // Show all info items for analog
    });
    
    // Show circuit diagram for analog levels
    const circuitDiagram = document.getElementById('circuitDiagram');
    if(circuitDiagram){
      circuitDiagram.style.display = 'block';
      // Trigger initial circuit draw
      if(circuitCtx){
        const R1 = Number(els.r1.value);
        const R2 = Number(els.r2.value);
        const vout = NoteGateSim.computeVout(Vin, R1, R2);
    ...
    console.log('📺 Showing analog controls (resistors, sliders, circuit diagram)');
  }
  ...
  function showDigitalControls(level, idx){
    ...
    // Hide analog circuit diagram for digital levels
    const circuitDiagram = document.getElementById('circuitDiagram');
    if(circuitDiagram) circuitDiagram.style.display = 'none';
    
    // Hide analog info (Vin, Target Vout, etc.)
    const infoItems = document.querySelectorAll('.info > div');
    infoItems.forEach((item, index) => {
      // Hide Vin, Target Vout, Tolerance, Vout (last 4 items)
      if(index >= infoItems.length - 4){
        item.style.display = 'none';
      }
    });
    
    console.log('🔌 Showing digital controls (logic gates)');
  ...
  // Test Circuit button
  const testCircuitBtn = document.getElementById('testCircuit');
  if(testCircuitBtn){
    testCircuitBtn.addEventListener('click', function(){
      if(circuitCtx){
        const R1 = Number(els.r1.value);
        const R2 = Number(els.r2.value);
        const vout = NoteGateSim.computeVout(Vin, R1, R2);
        updateCircuitDiagram(R1, R2, vout);
        showToast('🔬 Circuit simulation running!');
      } else {
        alert('Circuit canvas not available. Make sure you are on an analog level.');
      }
    });
  }
  ...
  if(els.skipLevelBtn){
    els.skipLevelBtn.addEventListener('click', skipLevel);
  }
  ...
  if(resetProgressBtn){
    resetProgressBtn.addEventListener('click', function(){
      if(confirm('🔄 Are you sure you want to reset all progress?\n\nThis will:\n• Start from Level 1\n• Reset Score to 0\n• Keep your High Score\n\nContinue?')){
        localStorage.setItem('noteGateCurrentLevel', 1);
        localStorage.setItem('noteGateScore', 0);
        const unlocked = [1]; // Only Level 1 unlocked
        localStorage.setItem('noteGateUnlocked', JSON.stringify(unlocked));
        console.log('🔄 Progress reset! Reloading...');
        showToast('🔄 Progress reset! Starting from Level 1...');
  ...
  // initialize
  // Load saved progress - start from last completed level
  const savedLevel = parseInt(localStorage.getItem('noteGateCurrentLevel')) || 1;
  currentLevel = Math.min(savedLevel, levels.length);
  console.log('💾 Loaded saved progress: Level ' + currentLevel);
  
  if(els.level) els.level.textContent = currentLevel;
  
  // Load saved score
  const savedScore = parseInt(localStorage.getItem('noteGateScore')) || 0;
  score = savedScore;
  els.score.textContent = score;
  console.log('💰 Loaded saved score: ' + score);
  
  // load high score
  const hsVal = localStorage.getItem('noteGateHighScore') || 0; 
  document.getElementById('highscore').textContent = hsVal;
  ...
  // Auto-unlock all levels up to current level
  ...
  localStorage.setItem('noteGateUnlocked', JSON.stringify(unlocked));
  console.log('🔓 Unlocked levels 1-' + currentLevel);
  
  // Show welcome message with progress
  if(currentLevel > 1){
    showToast('👋 Welcome back! Continuing from Level ' + currentLevel + ' (Score: ' + score + ')');
  } else {
    showToast('🎮 Welcome to Note Gate! Let\'s start Level 1!');
  }
